...
async def post_job(
    ...
    return JobCreateOut(job_id=job.id, queued_items=job.total_items, estimated_seconds=job.total_items * 30)


@router.get("", response_model=list[JobOut])
async def get_jobs(auth: AuthUser = Depends(verify_supabase_jwt), db: AsyncSession = Depends(get_db)) -> list[JobOut]:
    return [job_out(job) for job in await list_jobs(db, auth.id)]


@router.get("/{job_id}", response_model=JobOut)
async def get_job(job_id: UUID, auth: AuthUser = Depends(verify_supabase_jwt), db: AsyncSession = Depends(get_db)) -> JobOut:
    return job_out(await get_owned_job(db, auth.id, job_id))


@router.get("/{job_id}/items", response_model=list[JobItemOut])
async def get_job_items(job_id: UUID, auth: AuthUser = Depends(verify_supabase_jwt), db: AsyncSession = Depends(get_db)) -> list[JobItemOut]:
    job = await get_owned_job(db, auth.id, job_id)
    return [JobItemOut.model_validate(item, from_attributes=True) for item in sorted(job.items, key=lambda item: item.ordinal)]


@router.post("/{job_id}/cancel", response_model=JobOut)
async def post_cancel(job_id: UUID, auth: AuthUser = Depends(verify_supabase_jwt), db: AsyncSession = Depends(get_db)) -> JobOut:
    return job_out(await cancel_job(db, auth.id, job_id))


@router.get("/{job_id}/events")
async def job_events(job_id: UUID, auth: AuthUser = Depends(verify_supabase_jwt), db: AsyncSession = Depends(get_db)) -> StreamingResponse:
